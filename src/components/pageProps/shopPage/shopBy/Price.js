...
const Price = ({ selectedPriceRange, onPriceRangeChange }) => {
  const priceList = [
    {
      _id: 950,
      priceOne: 0.0,
  ...
  const handlePriceClick = (priceRange) => {
    if (selectedPriceRange && 
        selectedPriceRange.min === priceRange.priceOne && 
        selectedPriceRange.max === priceRange.priceTwo) {
      onPriceRangeChange(null); // Deselect if already selected
    } else {
      onPriceRangeChange({
        min: priceRange.priceOne,
        max: priceRange.priceTwo
      });
    }
  };
    ...
    <div className="cursor-pointer">
      ...
      <div className="font-titleFont">
        <ul className="flex flex-col gap-4 text-sm lg:text-base text-[#767676]">
          {priceList.map((item) => {
            const isSelected = selectedPriceRange && 
